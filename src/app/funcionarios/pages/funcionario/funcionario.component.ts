...
export class FuncionarioComponent implements OnInit {
  ...
  funcionario!: Funcionario //3 criado variavel para receber interace 
  ...
  constructor(
    private route: ActivatedRoute, // acessar os parâmetros da rota ativa 1º
    private funcService: FuncionarioService, //2 injetado
  ...
  ngOnInit(): void {
    // let idFuncionario = this.route.snapshot.paramMap.get('idFuncionario') rota que foi def no rout
    this.route.paramMap.subscribe(//quando subrota ideal utilizar paramMap (Obsvable)
      (params) => {
        let idFuncionario = parseInt(params.get('idFuncionario') ?? '0')
        //se o valor da esq não for nulo ou indefined vai utilizar o valor da esquerda, se não vai utilizar o valor da direita ??
